...
/*自动加载语言包*/
wmcms.auto(document.scripts);

/*表单验证规则，可修改*/
wmcms.rules=function()
{
	return ui_rule;
};

wmcms.language=function()
{
	return ui_lan;
};

wmcms.success=function(text,align,title,color,progress,time)
{
	$.success({title:title,text:text,align:align,color:color,progress:progress,time:time});
};

wmcms.error=function(text,align,title,color,progress,time)
{
	$.error({title:title,text:text,align:align,color:color,progress:progress,time:time});
};

wmcms.warn=function(text,align,title,color,progress,time)
{
	$.warn({title:title,text:text,align:align,color:color,progress:progress,time:time});
};

wmcms.loading=function(text,align,title,color,progress,time)
{
...
$(function()
{
    ...
    $formcheck=function(element,type,value,label)
	{
		...
		else
		{
			if(type=='checked')
			{
				...
				if(radio==0)
				{
					return wmcms.language()['checked']+label;
				}
				else
				{
					return true;
			...
			if(parts)
			{
				...
				switch(method)
				{
					case "length":
						...
						var where=params;
						var text=wmcms.language()['length']+params;
						if($test.length==2)
						{
							 where=$test[0];
							 text=$test[1];
					...
					case "min":
						...
						{
							return wmcms.language()['min']+params;
						}
						break;
					case "max":
						if(parseFloat($value)>parseFloat(params))
						{
							return wmcms.language()['max']+params;
						}
						break;
					case "between":
						...
						{
							return $.format(wmcms.language()['between'],[$min,$max])
						}
						break;
					case "match":
						var old=$("#"+params);
						...
						if(oldval!=$value)
						{
							return $.format(wmcms.language()['match'],[label,oldlabel])
						}
						break;
					case "ajax":
						var result='';
	...
	$.extend(
	{
		...
		dialog:function(opt)
		{
			...
			var defaults=
			{
				...
				time:0,
				ok:null,
				okval:wmcms.language()['ok'],
				oktheme:'ui-btn-blue',
				cancel:null,
				cancelval:wmcms.language()['cancel'],
				cancelshow:true,
				width:'',
				height:'',
				inputval:'',
	...
	$.fn.inputnumber=function(opt)
	{
		...
		this.each(function()
		{
			...
			/*默认文本为只读，不可以修改，除非配置：wmcms.inputnumber=true;
			if(wmcms.inputnumber==undefined)
			{
	...
	$.fn.backtime=function(opt)
	{
		...
		var backgo=function(t0)
		{
			...
			e['html']("<span class='ui-text-red ui-mr-sm'>"+t0+"</span>"+wmcms.language()['backtime']+"");
			t0=t0-1;
			if(t0>=0)
			{
			...
			else
			{
				setInterval(function()
				{
					$(e).each(function()
					{
						...
						if(myD>0)
						{
							str="<i>"+myD+"</i>"+wmcms.language()['day']+"<i>"+myH+"</i>"+wmcms.language()['hour']+"<i>"+myM+"</i>"+wmcms.language()['minute']+"<i>"+myS+"</i>"+wmcms.language()['second']+"";
						}
						else if(myH> 0)
						{
							str="<i>"+myH+"</i>"+wmcms.language()['hour']+"<i>"+myM+"</i>"+wmcms.language()['minute']+"<i>"+myS+"</i>"+wmcms.language()['second']+"";
						}
						else if(myM>0)
						{
							str="<i>"+myM+"</i>"+wmcms.language()['minute']+"<i>"+myS+"</i>"+wmcms.language()['second']+"";
						}
						else
						{
							str="<i>"+myS+"</i>"+wmcms.language()['second']+"";
						}
						if(nMS>0)
						{
							that.html(str);
	...
	$.fn.page=function(opt)
	{
		var defaults=
		{
			...
			thispage:1,
			num:3,
			total:wmcms.language()['total'],
			pre:wmcms.language()['pre'],
			next:wmcms.language()['next'],
			home:wmcms.language()['home'],
			last:wmcms.language()['last'],
			clickpage:1,
			ismobile:0,
			callback:function(){}
		};
